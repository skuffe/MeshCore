...
// Console packet feed. Decoupled from MESH_PACKET_LOGGING: upstream Dispatcher
// hardcodes its console lines to Serial, so we re-emit them (verbatim) from the
// logRx/logTx hooks to whatever stream PACKET_LOG_STREAM names (EthConsole here),
// keeping src/Dispatcher.cpp pristine. See helpers/PacketLogConsole.h.
#ifdef PACKET_LOG_STREAM
  #include <helpers/PacketLogConsole.h>
  #ifdef PACKET_LOG_STREAM_HEADER
    #include PACKET_LOG_STREAM_HEADER
  #endif
#endif
...
void MyMesh::logRxRaw(float snr, float rssi, const uint8_t raw[], int len) {
  ...
  if (cliext::g_packet_dump_enabled) {
    PACKET_LOG_STREAM.print(getLogDateTime());
    PACKET_LOG_STREAM.print(" RAW: ");
    mesh::Utils::printHex(PACKET_LOG_STREAM, raw, len);
...
#ifdef PACKET_LOG_STREAM
  // Console feed (independent of file _logging) — reproduces the upstream
  // Dispatcher console line that now no longer fires (pristine Dispatcher prints
  // to Serial). Byte-identical so the bridge on :5000 keeps parsing. Gated on the
  // runtime observation toggle (`log on|off`).
  if (cliext::g_packet_dump_enabled)
...
void MyMesh::handleCommand(uint32_t sender_timestamp, char *command, char *reply) {
  ...
  if (memcmp(command, "setperm ", 8) == 0) {   // format:  setperm {pubkey-hex} {permissions-int8}
    ...
    reply[0] = 0;
  } else if (sender_timestamp == 0 && strcmp(command, "stats") == 0) {
    // text variant of REQ_TYPE_GET_STATUS, for console/MQTT-bridge consumers
    sprintf(reply, "bat=%d nf=%d up=%lu txair=%lu rxair=%lu queue=%d recv=%lu sent=%lu",
        (int)board.getBattMilliVolts(), (int)_radio->getNoiseFloor(),
        (unsigned long)(uptime_millis / 1000),
